...
class TwoPhaseSimplex(StandardSimplex):
    def __init__(self, problem: LPProblem):
        ...
        # Phase labels stored alongside history entries so the UI can group them
        self._phase_labels = []
    ...
    def _run_simplex_loop(self, history: list = None, phase_label: str = '') -> str:
        ...
        if history is not None:
            history.append(self._snapshot(step))
    ...
    def solve(self) -> dict:
        ...
        status = self._run_simplex_loop(history=history, phase_label='Phase 1')
        ...
        if transition_status == 'infeasible':
            return {'status': 'infeasible', 'solution': None, 'z': None, 'history': history,
                    'phase_labels': self._phase_labels}

        status = self._run_simplex_loop(history=history, phase_label='Phase 2')
